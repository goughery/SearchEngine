# I427 Fall 2015, Final project
#   Code author: Shaowen Ren(shaoren)
#   
#   based on skeleton code by D Crandall

#Search engine URL: http://cgi.soic.indiana.edu/~shaoren/gough-ren-search.html

# The result may show "duplicated" pages. They have the same title, but the url is a little different.
# I think it is just a problem of the crawled data that been provided for the Assignment 3.
...
def retrieve(searchList,docindex,invindex,pagerank)
  ...
  allHits.each do |document| #calculate score for each document(hit)
    ...
    hits_score[document] = tfidf_score * (Math.atan(pagerank_score * 4))/30 #score calculate method
  end	
...
def hits_display_cgi(ranked_hits, hits_score, search_through, docindex) #result displaying method
  ...
  ranked_hits.each do |hit|	
    ...
    single_output.push(hits_score[hit])#tf-idf * pagerank score
    count += 1
